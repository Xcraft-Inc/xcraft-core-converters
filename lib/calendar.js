...
/**
 * @param {{date: string, time?: string | null, offset?: string, timezone?: string, tags?: string}} parts
 * @returns {string}
 */
...
function dateToTimezone(date, timezone) {
  ...
  if (!fractionalSecond || fractionalSecond === '000') {
...
function plainTimeISO(date = new Date()) {
  ...
  if (milliseconds > 0) {
    millisecondsText = '.' + milliseconds.toString().padStart(3, '0');
  }
  return `${hours}:${minutes}:${seconds}${millisecondsText}`;
}
...
/**
 * @param {'minute' | 'second' | undefined} [precision]
 */
function nowZonedDateTimeISO(precision) {
  const now = new Date();
  if (precision === 'minute') {
    now.setSeconds(0, 0);
  ...
  const plainDateTime = plainDateTimeISO(now);
...
function addDateDuration(date, duration) {
  date = new Date(date);
  if (duration.weeks) {
    date.setDate(date.getDate() + 7 * duration.weeks);
  }
  if (duration.days) {
 ...
 * @returns {string}
 */
function addDuration(dateOrDateTime, duration) {
  const {time, timezone} = parseZonedDateTime(dateOrDateTime);

  const date = addDateDuration(toJsDate(dateOrDateTime), duration);
