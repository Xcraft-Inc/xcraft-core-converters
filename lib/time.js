...
function getDisplayed(time, format, acceptEmpty) {
  if (!time || (!acceptEmpty && isEmpty(time))) {
    return null;
  }
  const d = split(time);
  if (d) {
...
function getPeriodDescription(fromTime, toTime, format) {
  if (fromTime && toTime) {
    ...
    if (fh === th && fm === tm) {
      return getDisplayed(fromTime, null, true);
    } else {
      if (format === 'ft') {
        return (
          getDisplayed(fromTime, null, true) +
          ' ' +
          getDisplayed(toTime, null, true)
        );
      } else if (format === 'f-t') {
        return (
          getDisplayed(fromTime, null, true) +
    ...
    return getDisplayed(fromTime || toTime, null, true);
  }
}
