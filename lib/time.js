...
function getDisplayed(time, format) {
  ...
  if (d) {
    if (format === 'hms') {
      ...
      return padding(d.hours, 2) + 'h' + padding(d.minutes, 2);
    } else if (format === 'h') {
      return padding(d.hours, 2);
    } else if (format === 'Hm') {
      let h = '';
      h = StringBuilder.joinWords(
        d.hours,
        d.hours <= 1 ? T('heure') : T('heures')
      );
      let m = '';
      if (d.minutes > 0) {
        m = d.minutes + '';
      ...
      if (h && m) {
        return StringBuilder.joinWords(h, m);
      } else {
        return StringBuilder.combine(h, m);
      }
    ...
    if (isNaN(edited[0])) {
      ...
      if (edited[0] < 24) {
        ...
        if (edited.length === 1) {
          ...
          const m = edited[0] % 100;
          const h = Math.floor(edited[0] / 100);
          if (h < 0 || h >= 24) {
            incorrectHour = true;
          } else if (m < 0 || m > 59) {
  ...
  if (fromTime && toTime) {
    ...
    if (fh === th && fm === tm) {
      ...
      if (format === 'ft') {
        ...
        return getDisplayed(fromTime) + ' — ' + getDisplayed(toTime);
      } else if (format === 'fh - th') {
        return (
          getDisplayed(fromTime, '..h..') +
          ' – ' +
          getDisplayed(toTime, '..h..')
        );
      } else {
        return getDisplayed(fromTime) + ' → ' + getDisplayed(toTime);
      }
...
function getDisplayedBetweenTwoTimes(time1, time2, format) {
  ...
  const d = [];
  if (h > 0) {
    d.push(StringBuilder.joinWords(h, h <= 1 ? T('heure') : T('heures')));
  }
  if (m > 0) {
    d.push(StringBuilder.joinWords(m, m <= 1 ? T('minute') : T('minutes')));
  }
  if (format === 'hms' && s > 0) {
    d.push(StringBuilder.joinWords(s, s <= 1 ? T('seconde') : T('secondes')));
  }
  const t = StringBuilder.joinWords(d);

  if (n === 0) {
    return T('time|between-two-times|Maintenant');
  } else if (n > 0) {
    return StringBuilder.joinWords(T('time|between-two-times|Dans'), t);
  } else if (n < 0) {
    return StringBuilder.joinWords(T('time|between-two-times|Dépassé de'), t);
  }
}
...
function incEdited(editedDate, cursorPosition, inc) {
  ...
  if (!error) {
    if (cursorPosition <= 2) {
      // [12]:00:00
      newTime = addHours(value, inc);
      selectionStart = 0;
      selectionEnd = 0 + 2;
    } else {
      // 12:[00]:00
      newTime = addMinutes(value, inc);
      selectionStart = 3;
      selectionEnd = 3 + 2;
    }
  }
