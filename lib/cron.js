...
function hasRecurrence (startDate, endDate, days, addDates, date) {
  if (date < startDate || date > endDate) {
    return false; // out of period
  }
  const isInsideAdd = addDates.indexOf (date) !== -1;
  const dates = computeCronDates (date, date, days); // generate only one date, it's enough
...
function getDatesSummary (startDate, endDate, days, addDates, type) {
  const dates = computeCronDates (startDate, endDate, days);
  const array = [];
