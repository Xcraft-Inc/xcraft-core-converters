...
function adjustEndDate (startDate, endDate, maxDays) {
  maxDays = maxDays || 365;
  const daysCount = DateConverters.getDaysBetweenTwoDates (startDate, endDate);
  if (daysCount > maxDays) {
    endDate = DateConverters.addDays (startDate, maxDays);
  }
  return endDate;
}
...
exports.adjustEndDate = adjustEndDate;
