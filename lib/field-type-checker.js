...
const PixelConverters = require('./pixel.js');
...
const ColorConverters = require('./color.js');
...
function checkEntityId(fieldName, fieldContent, typedef) {
  if (!typedef.target) {
    return {
      ok: false,
      message: `In field "${fieldName}", the type "${typedef.type}" has no "target".`,
      color: 'black',
      glyph: 'solid/bug',
    };
  }
...
function checkEnum(fieldName, fieldContent, typedef) {
  if (!typedef.values) {
    return {
      ok: false,
      message: `In field "${fieldName}", the type "${typedef.type}" has no "values".`,
      color: 'black',
      glyph: 'solid/bug',
    };
  }
...
function check(fieldName, fieldContent, typedef) {
  ...
  switch (type) {
    ...
    case 'length':
      ...
      break;
    case 'pixel':
      result.ok = PixelConverters.check(fieldContent);
      break;
    case 'volume':
    ...
    case 'delay':
      ...
      break;
    case 'color':
      result.ok = ColorConverters.check(fieldContent);
      break;
    default:
      return {
        ok: false,
