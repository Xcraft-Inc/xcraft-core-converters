...
function changePeriod(fromDate, toDate, direction) {
  ...
  if (fd === 1 && fm === 1 && td === 31 && tm === 12) {
    // 01.01.xxxx -> 31.12.xxxx (full year)
    const yCount = ty - fy + 1;
    fromDate = addYears(fromDate, yCount * direction);
    toDate = addYears(toDate, yCount * direction);
  } else if (fd === 1 && td === endOfMonth && fm === tm && fy === ty) {
    // 01.03.2020 -> 31.03.2020 (full month)
    // TODO: Faire juste !
    fromDate = addMonths(fromDate, direction);
    toDate = addMonths(toDate, direction);
  } else {
