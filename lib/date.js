...
function getDisplayed(canonicalDate, format, acceptEmpty) {
  if (!canonicalDate || (!acceptEmpty && isEmpty(canonicalDate))) {
    return null;
  }
...
function getPeriodDescription(fromDate, toDate) {
  ...
  var fm = getDisplayed(fromDate, 'M', true);
  var fy = getDisplayed(fromDate, 'y', true);

  var td = getDay(toDate);
  var tm = getDisplayed(toDate, 'M', true);
  var ty = getDisplayed(toDate, 'y', true);
