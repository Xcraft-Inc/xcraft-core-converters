...
function incEdited(editedDate, cursorPosition, inc) {
  let newDate = null;
  let selectionStart = -1;
  let selectionEnd = -1;

  // value = "31.03.2020"
  const {value, error} = this.parseEdited(editedDate);
  if (!error) {
    if (cursorPosition <= 2) {
      // [31].03.2020
      newDate = addDays(value, inc);
      selectionStart = 0;
      selectionEnd = 0 + 2;
    } else if (cursorPosition <= 5) {
      // 31.[03].2020
      newDate = addMonths(value, inc);
      selectionStart = 3;
      selectionEnd = 3 + 2;
    } else {
      // 31.03.[2020]
      newDate = addYears(value, inc);
      selectionStart = 6;
      selectionEnd = 6 + 4;
    }
  }
