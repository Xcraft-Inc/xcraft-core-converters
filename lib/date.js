...
function canonicalToJs(date) {
  if (typeof date === 'string') {
    ...
    throw new Error(`DateConverter: Bad date '${date}'`);
  }
}
...
// Add months, with 'intelligent' computing for dates at the end of a month.
function addMonths(date, n) {
  const initialDate = date;

  const jsD = canonicalToJs(date);
  const y = jsD.getFullYear(); // 2017..
  const m = jsD.getMonth() + 1; // 1..12
  const d = jsD.getDate(); // 1..31

  const maxDay = getDay(addDays(getDate(y, m + n + 1, 1), -1)); // 28..31
  const newDay = Math.min(d, maxDay);
  const newMonth = ((m + n - 1) % 12) + 1;
  const newYear = y + Math.trunc((m + n - 1) / 12);
  const nd = new Date(newYear, newMonth - 1, newDay);
  date = jsToCanonical(nd);

  if (n < 0) {
    const endOfMonth = addDays(getDate(y, m + 1, 1), -1);
    if (endOfMonth === initialDate) {
      date = getDate(newYear, newMonth, 1);
      date = moveAtEndingOfMonth(date);
    }
  }
...
function addYears(date, n) {
  ...
  const day = d.getDate();
  let newDate = new Date(year + n, month, day);
  if (newDate.getMonth() > month) {
    newDate = new Date(year + n, month, day - 1);
  }
  return jsToCanonical(newDate);
}
...
function split(canonicalDate) {
  ...
  if (
    ...
    throw new Error(
      `DateConverter: Bad canonical date '${canonicalDate}' (must be 'yyyy-mm-dd')`
    );
  }
...
function getDisplayed(canonicalDate, format, shift) {
  if (!canonicalDate) {
    return '';
  }

  if (shift) {
    canonicalDate = addDays(canonicalDate, shift);
  }
...
function getPeriodDescription(fromDate, toDate, format, separator, shift) {
  if (shift && toDate) {
    toDate = addDays(toDate, shift);
  }
  separator = separator || ' \u2192 '; // \u2192 = right arrow "→"

  if (format === 'compact') {
    const f = getDisplayed(fromDate) || '-∞';
    const t = getDisplayed(toDate) || '∞';
...
// date='2020-01-01' exp='1y/-1d' -> return '2020-12-31'
function getCalcDate(date, exps, direction = 1) {
  if (date && exps && exps.length > 1) {
    for (const exp of exps.split('/')) {
      const n = parseInt(exp.substring(0, exp.length - 1)) * direction;
...
function incEdited(edited, cursorPosition, direction, step = 1, min, max) {
  let newDate = null;
  ...
  if (!error) {
    if (cursorPosition <= 2) {
      // [31].03.2020
      newDate = addDays(value, direction * step);
      selectionStart = 0;
      selectionEnd = 0 + 2;
    } else if (cursorPosition <= 5) {
      // 31.[03].2020
      newDate = addMonths(value, direction * step);
      selectionStart = 3;
      selectionEnd = 3 + 2;
    } else {
      // 31.03.[2020]
      newDate = addYears(value, direction * step);
      selectionStart = 6;
      selectionEnd = 6 + 4;
    }
  }
  ...
  return {
    edited: getDisplayed(newDate),
    selectionStart,
    selectionEnd,
  };
}

function changePeriod(fromDate, toDate, direction) {
  if (direction !== 1 && direction !== -1) {
    throw new Error(`DateConverter: Bad direction '${direction}'`);
  }

  const f = canonicalToJs(fromDate);
  const fy = f.getFullYear(); // 2017..
  const fm = f.getMonth() + 1; // 1..12
  const fd = f.getDate(); // 1..31

  const t = canonicalToJs(toDate);
  const ty = t.getFullYear(); // 2017..
  const tm = t.getMonth() + 1; // 1..12
  const td = t.getDate(); // 1..31

  const endOfMonth = addDays(getDate(ty, tm + 1, 1), -1);
  let message; // for debug

  if (fd === 1 && fm === 1 && td === 31 && tm === 12) {
    // 01.01.2020 -> 31.12.2020 (whole year)
    // 01.01.2020 -> 31.12.2021 (whole years)
    const yCount = ty - fy + 1;
    message = `yCount=${yCount}`;
    fromDate = addYears(fromDate, yCount * direction);
    toDate = addYears(toDate, yCount * direction);
  } else if (fd === 1 && toDate === endOfMonth) {
    // 01.03.2020 -> 31.03.2020 (whole month)
    // 01.03.2020 -> 30.04.2020 (whole months)
    // 01.03.2020 -> 28.02.2021 (whole months)
    const mCount = tm + ty * 12 - (fm + fy * 12) + 1;
    message = `mCount=${mCount}`;
    if (direction > 0) {
      fromDate = addDays(toDate, 1);
      toDate = addDays(addMonths(fromDate, mCount), -1);
    } else {
      toDate = addDays(fromDate, -1);
      fromDate = addMonths(fromDate, -mCount);
    }
  } else {
    const days = (t - f) / (1000 * 60 * 60 * 24) + 1;
    if (direction > 0) {
      fromDate = addDays(toDate, 1);
      toDate = addDays(toDate, days);
    } else {
      toDate = addDays(fromDate, -1);
  ...
  return {fromDate, toDate, message};
}
