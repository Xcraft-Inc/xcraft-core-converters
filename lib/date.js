...
function getDOWDescription(dow, format) {
  ...
  if (format === '3') {
    ...
    return toFirstUpperCase(array[dow].substring(0, 3));
  } else if (format === 'firstUpperCase') {
    return toFirstUpperCase(array[dow]);
  } else {
    return array[dow];
  }
}
...
// If format contains 's' (short), the result never has the sign '∞'.
// If format contains '3', months are formatted with 3 characters.
// Examples with format = null:
//    "5 octobre 2016 → 31 décembre ∞"
// Examples with format = 's':
//    "5 octobre 2016"
// Examples with format = 's3':
//    "5 oct 2016"
//    "12 mar 2016 → 24 jui 2017"
//    "Oct 2016 → fév 2017"
function getPeriodDescription(fromDate, toDate, format) {
  ...
  const f = fy === '-∞' && withoutInfinity ? '' : join(fromList, ' ');
  const t = ty === '∞' && withoutInfinity ? '' : join(toList, ' ');

  if (f === '') {
    return toFirstUpperCase(t);
  } else if (t === '') {
    return toFirstUpperCase(f);
  } else {
