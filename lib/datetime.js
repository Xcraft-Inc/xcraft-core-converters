...
function getDisplayed(canonicalDateTime, format, acceptEmpty) {
  if (!canonicalDateTime || (!acceptEmpty && isEmpty(canonicalDateTime))) {
    return null;
  }
...
function getPeriodDescription(fromDatetime, toDatetime) {
  ...
  if (fromDate === toDate) {
    const date = DateConverters.getDisplayed(fromDate, 'dMy', true);
...
function getDisplayedBetweenToDatetimes(datetime1, datetime2) {
  ...
  const time =
    padding(hours, 2) + ':' + padding(minutes, 2) + ':' + padding(seconds, 2);
  const timeDuration =
    TimeConverters.getDisplayed(time, 'duration', true) || '';
  if (days === 0) {
