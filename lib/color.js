...
function _dec2hex(i) {
  return (Math.trunc(i || 255) + 0x100).toString(16).substr(-2).toUpperCase();
}

function _toString(value, defaultValue = 0) {
...
function _toCanonicalRGB(rgb) {
  const r = _dec2hex(rgb.r);
  const g = _dec2hex(rgb.g);
  const b = _dec2hex(rgb.b);
  return `#${r}${g}${b}`;
}

function _toCanonicalCMYK(cmyk) {
  const c = _toString(cmyk.c, 0);
  const m = _toString(cmyk.m, 0);
  const y = _toString(cmyk.y, 0);
  const k = _toString(cmyk.k, 0);
  return `CMYK(${c},${m},${y},${k})`;
}

function _toCanonicalHSL(hsl) {
  const h = _toString(hsl.h, 0);
  const s = _toString(hsl.s, 100);
  const l = _toString(hsl.l, 100);
  return `HSL(${h},${s},${l})`;
}

function _toCanonicalGrey(g) {
  const n = _toString(g.n, 0);
  return `G(${n})`;
}
