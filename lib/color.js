...
function _dec2hex(value, defaultValue) {
  if (value === undefined || value === null) {
    value = defaultValue;
  }
  return (Math.trunc(value) + 0x100).toString(16).substr(-2).toUpperCase();
}

function _dec2string(value, defaultValue = 0) {
  if (value === undefined || value === null) {
    return defaultValue + '';
  } else {
    return value + '';
...
function _toCanonicalRGB(rgb) {
  const r = _dec2hex(rgb.r, 255);
  const g = _dec2hex(rgb.g, 255);
  const b = _dec2hex(rgb.b, 255);
  return `#${r}${g}${b}`;
}

function _toCanonicalCMYK(cmyk) {
  const c = _dec2string(cmyk.c, 0);
  const m = _dec2string(cmyk.m, 0);
  const y = _dec2string(cmyk.y, 0);
  const k = _dec2string(cmyk.k, 0);
  return `CMYK(${c},${m},${y},${k})`;
}

function _toCanonicalHSL(hsl) {
  const h = _dec2string(hsl.h, 0);
  const s = _dec2string(hsl.s, 100);
  const l = _dec2string(hsl.l, 100);
  return `HSL(${h},${s},${l})`;
}

function _toCanonicalGrey(g) {
  const n = _dec2string(g.n, 0);
  return `G(${n})`;
}

// Returns a normalized angle, i.e. between 0 and 360°.
function _clipAngleDeg(angle) {
  angle = angle % 360.0;
...
function changeColor(
  ...
  const analysis = analysisFromCanonical(color);
  const hue = _clipAngleDeg(analysis.h + hueShift);
  ...
  return toRGB(analysisToCanonical(newAnalysis));
}
