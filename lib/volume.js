...
// The cm3kg (cm3/kg) is currently equal to 5000, but Velocity use 6000!
...
function getCanonicalIATA (canonicalVolume, cm3kg) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    iata = iata.mul (convert (value, 'm', 'cm'));
  }
  return iata.div (cm3kg || 6000).toString ();
}

//-----------------------------------------------------------------------------
// Return volumetric weight in kg with 3 decimals (for 'gram' precision).
function getDisplayedIATA (canonicalVolume, mass) {
  BigNumber.config ({FORMAT: parseFormat, ERRORS: false});
  const iata = new BigNumber (getCanonicalIATA (canonicalVolume, mass));
...
function getDisplayed (canonicalVolume, displayedUnit) {
  ...
  const result = [];
  for (let i = 0; i < array.length; i++) {
    const value = convert (array[i], 'm', displayedUnit);
...
function parseEdited (editedVolume, editedUnit) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    result.push (convert (value, unit, 'm').toString ());
  }
