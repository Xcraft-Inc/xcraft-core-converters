...
function convertVolume(value, inputUnit, outputUnit, decimals) {
  ...
  value = new BigNumber(value);
  value = value.times(unitsOfVolume[inputUnit]); // to canonical unit
...
function getCanonicalIATA(canonicalVolume, cm3kg) {
  ...
  if (array.length === 1) {
    ...
    for (let i = 0; i < array.length; i++) {
      iata = iata.times(convertLength(array[i], 'm', 'cm'));
    }
  }
...
function getDisplayed(canonicalVolume, displayedUnit) {
  ...
  if (p.error) {
    return StringBuilder.joinWords(T('Erreur'), `(${canonicalVolume})`);
  }
