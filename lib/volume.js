...
function getDisplayed (canonicalVolume, displayedUnit) {
  ...
  if (array.length === 1) {
    // One value -> volume.
    const value = convertVolume (array[0], 'm3', volumeUnit);
    return value + volumeUnit;
  } else {
    // Three values -> dimensions.
    const result = [];
    for (let i = 0; i < array.length; i++) {
      const value = convertLength (array[i], 'm', lengthUnit);
      result.push (value.toString ());
    }
    return result.join (' × ') + ' ' + lengthUnit; // U+00D7 (signe multiplication)
  }
}
  ...
  if (editedUnit === '') {
    ...
    if (defaultUnit) {
      ...
      if (units.length === 1) {
        ...
        throw new Error (`Invalid units ${defaultUnit}`);
      }
    }
  } else {
